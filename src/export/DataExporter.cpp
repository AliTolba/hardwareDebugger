                          ...
                          const std::function<void(const QString&)>& xOnError)
{
    ...
    sPublishProgress(xEventBus, lTaskName, -1);
                                ...
                                const std::function<void(const QString&)>& xOnError)
{
    ...
    auto* lThread = QThread::create([lRemainingBatches, lContext, lModelGuard, xEventBus, lTaskName, xStrategy, xFilePath, xParent, xOnSuccess, xOnError]() {
        for (int lStep = 0; lStep < lRemainingBatches; ++lStep) {
            // Invoke fetchMore on UI thread
            QMetaObject::invokeMethod(lContext, [lModelGuard]() {
            ...
            // Update progress
            const int lProgress = ((lStep + 1) * 100) / lRemainingBatches;
            QMetaObject::invokeMethod(xEventBus, [xEventBus, lTaskName, lProgress]() {
                sPublishProgress(xEventBus, lTaskName, lProgress);
            }, Qt::QueuedConnection);
        }
    });

    lContext->connect(lThread, &QThread::finished, lContext, [lModelGuard, xStrategy, xFilePath, xParent, xEventBus, xOnSuccess, xOnError, lTaskName]() {
    ...
    lContext->connect(lThread, &QThread::finished, lThread, &QObject::deleteLater);
