...
bool DatabaseManager::mPopulateDesignTable()
{
    ...
    int lId = 1;
    for (int lRoot = 0; lRoot < 40; ++lRoot) {
        const int lRootId = lId++;
        lQuery.bindValue(QStringLiteral(":id"), lRootId);
        lQuery.bindValue(QStringLiteral(":parent_id"), QVariant());
        lQuery.bindValue(QStringLiteral(":instance_name"), QStringLiteral("top_%1").arg(lRoot, 2, 10, QLatin1Char('0')));
        ...
        lQuery.bindValue(QStringLiteral(":child_count"), 24);
        if (!lQuery.exec()) {
            lDb.rollback();
            return false;
        }

        for (int lBranch = 0; lBranch < 24; ++lBranch) {
            ...
            lQuery.bindValue(QStringLiteral(":child_count"), 10);
            if (!lQuery.exec()) {
                lDb.rollback();
                return false;
            }

            for (int lLeaf = 0; lLeaf < 10; ++lLeaf) {
...
bool DatabaseManager::mPopulateSignalTable()
{
    ...
    int lId = 1;
    for (int lRoot = 0; lRoot < 30; ++lRoot) {
        const int lRootId = lId++;
        lQuery.bindValue(QStringLiteral(":id"), lRootId);
        lQuery.bindValue(QStringLiteral(":parent_id"), QVariant());
        ...
        lQuery.bindValue(QStringLiteral(":child_count"), 28);
        if (!lQuery.exec()) {
            lDb.rollback();
            return false;
        }

        for (int lBranch = 0; lBranch < 28; ++lBranch) {
            const int lBranchId = lId++;
            lQuery.bindValue(QStringLiteral(":id"), lBranchId);
            lQuery.bindValue(QStringLiteral(":parent_id"), lRootId);
            ...
            lQuery.bindValue(QStringLiteral(":child_count"), 12);
            if (!lQuery.exec()) {
                lDb.rollback();
                return false;
            }

            for (int lLeaf = 0; lLeaf < 12; ++lLeaf) {
                lQuery.bindValue(QStringLiteral(":id"), lId++);
                lQuery.bindValue(QStringLiteral(":parent_id"), lBranchId);
...
bool DatabaseManager::mPopulateAssertionTable()
{
    ...
    for (int lI = 1; lI <= 60000; ++lI) {
        const QString lStatus = randomFrom(sStates);
        lQuery.bindValue(QStringLiteral(":id"), lI);
        lQuery.bindValue(QStringLiteral(":property_name"), QStringLiteral("p_stable_%1").arg(lI % 300));
        lQuery.bindValue(QStringLiteral(":module_name"), QStringLiteral("top_%1/mod_%2").arg(lI % 40).arg(lI % 120));
        ...
        lQuery.bindValue(QStringLiteral(":message"), QStringLiteral("Property %1 sampled at stage %2").arg(lI % 300).arg(lI % 16));
...
bool DatabaseManager::mPopulateTransactionTable()
{
    ...
    for (int lI = 1; lI <= 90000; ++lI) {
        ...
        lQuery.bindValue(QStringLiteral(":address"), 0x1000 + (lI % 2048) * 4);
        lQuery.bindValue(QStringLiteral(":burst_length"), (lI % 16) + 1);
